...
CTexturedRectMesh::CTexturedRectMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth, float fHeight, float fDepth, float fxPosition, float fyPosition, float fzPosition, bool isFlip) : CMesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 6;
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
...
CPlayerHpMesh::CPlayerHpMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
	: CMesh(pd3dDevice, pd3dCommandList)
{
	...
	pVertices[0] = CTexturedVertex(XMFLOAT3(0.f, +fy, fz), XMFLOAT2(0.0f, 0.0f));
	pVertices[1] = CTexturedVertex(XMFLOAT3(0.f, -fy, fz), XMFLOAT2(0.0f, 1.0f));
	pVertices[2] = CTexturedVertex(XMFLOAT3(+fx, -fy, fz), XMFLOAT2(1.0f, 1.0f));
	pVertices[3] = CTexturedVertex(XMFLOAT3(+fx, -fy, fz), XMFLOAT2(1.0f, 1.0f));
	pVertices[4] = CTexturedVertex(XMFLOAT3(+fx, +fy, fz), XMFLOAT2(1.0f, 0.0f));
	pVertices[5] = CTexturedVertex(XMFLOAT3(0.f, +fy, fz), XMFLOAT2(0.0f, 0.0f));
...
{
	//CTexturedVertex* pVertices = new CTexturedVertex[6];
	////CTexturedVertex pVertices[6];

	//pVertices[0] = CTexturedVertex(xmf3Bottom2, XMFLOAT2(1.0f, 0.0f));
	//pVertices[1] = CTexturedVertex(xmf3Top2,	XMFLOAT2(1.0f, 1.0f));
	//pVertices[2] = CTexturedVertex(xmf3Top1,	XMFLOAT2(0.0f, 1.0f));
	//pVertices[3] = CTexturedVertex(xmf3Top1,	XMFLOAT2(0.0f, 1.0f));
	//pVertices[4] = CTexturedVertex(xmf3Bottom1, XMFLOAT2(0.0f, 0.0f));
	//pVertices[5] = CTexturedVertex(xmf3Bottom2, XMFLOAT2(1.0f, 0.0f));

	//memcpy(m_pBufferDataBegin, pVertices, sizeof(CTexturedVertex) * 6);
	//delete[] pVertices;
}
...
void CParticleMesh::CreateStreamOutputBuffer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
